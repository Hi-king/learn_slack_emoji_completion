...
def main(
    ...
    model = emojicompletion.model.Transformer(n_token=len(tokenizer.dictionary))
    candidates, case_dict = emojicompletion.data.SlackEmojiCompletionDataset(directory=pathlib.Path(__file__).parent / 'data').load(filter_by_vocabulary=True)
    candidates = list(candidates)
    keys_train, keys_test = sklearn.model_selection.train_test_split(list(case_dict.keys()), test_size=1-train_ratio)
    ...
    for epoch in range(num_epoch):
        ...
        model.train()
        for i, (x, y) in enumerate(data_generator(keys_train, desc=f'[train, epoch{epoch}]')):
            batch.append((x, y))
        ...
        confmat = np.zeros((2, 2), dtype=int)
        model.eval()
        for x, y in tqdm.tqdm(data_generator(keys_test, desc=f'[validation, epoch{epoch}]')):
            batch.append((x, y))
            if len(batch) >= batch_size:
                xs = [item[0] for item in batch]
                maxlen = max(len(x) for x in xs)
