...
def main(
    ...
    keys_train, keys_test = sklearn.model_selection.train_test_split(list(case_dict.keys()), test_size=0.2)
    optimizer = torch.optim.Adam(model.parameters(), lr=lr, weight_decay=weight_decay)


    def data_generator():
        for key in tqdm.tqdm(keys_train):
            if case_dict[key]:
